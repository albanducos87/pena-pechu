...
(function() {
   ...
   */
  window.addEventListener('load', () => {
    let galerieContainer = select('.galerie-container');
    if (galerieContainer) {
      let galerieIsotope = new Isotope(galerieContainer, {
        itemSelector: '.galerie-item',
        layoutMode: 'fitRows',
        filter: '.filter-filter-dupouy'
      });
      ...
      on('click', '#galerie-filters li', function(e) {
        ...
        galerieIsotope.arrange({
          filter: this.getAttribute('data-filter')
        });
        galerieIsotope.on('arrangeComplete', function() {
          AOS.refresh()
        });
      }, true);
    }
  ...
  /**
   * galerie details slider
   */
  new Swiper('.galerie-details-slider', {
    speed: 400,
    autoplay: {
      delay: 5000,
